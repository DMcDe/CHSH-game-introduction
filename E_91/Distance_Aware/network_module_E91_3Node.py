...
class Game:
    def __init__(self,debug: bool = False, fid: int = 1, freq: int = 1, eff: int = 1, 
                 wave: int = 1,guarantee_entanglement: bool = True,deltas = None,distance=10e1,attenuation = 0):
    ...
    #     self.qcA = QuantumChannel('qcA', self.tl, attenuation = 0.2, distance = self.distance)
    #     self.qcB = QuantumChannel('qcB', self.tl, attenuation = 0.2, distance = self.distance)
    ...
    def setup(self):
        ...
        # 1. ONLY 3 NODES: Alice, Bob, and the Entangler (Charlie)
        self.a = PlayerNode('a', self.tl, self.shared_circ, 0, fid=self.fidelity)
        self.b = PlayerNode('b', self.tl, self.shared_circ, 1, fid=self.fidelity)
        self.ent_node = BSMNode('ent_node', self.tl, ['a', 'b'])

        # Note: No 'ref' node is created here
        self.nodes = [self.a, self.b, self.ent_node]

        # 2. Quantum Channels (remains same)
        ...
        # 3. Classical Channels (Only between A, B, and Entangler)
        for i in range(len(self.nodes)):
            for j in range(len(self.nodes)):
                if i != j:
                    cc = ClassicalChannel(f'cc_{self.nodes[i].name}_{self.nodes[j].name}', self.tl, 1000, 1e8)
                    cc.set_ends(self.nodes[i], self.nodes[j].name)
